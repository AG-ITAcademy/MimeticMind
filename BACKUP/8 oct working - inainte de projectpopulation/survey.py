...
def set_survey_reload_flag(project_survey_id, value):
    r = redis.Redis(host=Config.REDIS_HOST, port=Config.REDIS_PORT, db=Config.REDIS_DB)
    r.set(f"survey_reload_flag_{project_survey_id}", value)


def get_survey_reload_flag(project_survey_id):
    r = redis.Redis(host=Config.REDIS_HOST, port=Config.REDIS_PORT, db=Config.REDIS_DB)
    flag = r.get(f"survey_reload_flag_{project_survey_id}")
    
    if flag is None:
        return 0  # Default to 0 if the flag does not exist
    return int(flag)
