...
class SurveyTemplate(db.Model):
    ...
    query_templates = db.relationship(
...
class ProjectPopulation(db.Model):
    __tablename__ = 'project_population'

    id = db.Column(db.Integer, primary_key=True)
    project_id = db.Column(db.Integer, db.ForeignKey('projects.id'), nullable=False)
    population_tag = db.Column(db.String(255), nullable=False)
    created_at = db.Column(db.DateTime, default=db.func.current_timestamp())
    
    project = db.relationship('Project', backref='populations')
...
class ProjectSurvey(db.Model):
    ...
    #survey_template = db.relationship('SurveyTemplate', back_populates='projects')
