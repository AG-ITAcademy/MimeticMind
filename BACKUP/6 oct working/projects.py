...
def project_dashboard(project_id):
    ...
    project_surveys = ProjectSurvey.query.filter_by(project_id=project_id).all()
...
def run_survey(project_id, survey_id):
    ...
    # Prevent survey re-run if it's still in progress
    if project_survey.completion_percentage is not None and project_survey.completion_percentage < 100:
        flash(f'Survey "{project_survey.survey_alias}" is already running.', 'warning')
        return redirect(url_for('projects_bp.project_dashboard', project_id=project_id))
    ...
    flash(f'Survey "{project_survey.survey_alias}" has been successfully run.', 'success')
    return redirect(url_for('projects_bp.project_dashboard', project_id=project_id))
...
def survey_progress(project_survey_id):
    ...
    progress = r.get(f"survey_progress_{project_survey_id}")
    if progress is None:
        progress = 0
